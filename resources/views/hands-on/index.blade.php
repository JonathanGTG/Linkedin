...
<div class="page-wrap">
  <div class="filter-row">
    ...
    <a class="reset" href="{{ route('hands-on.index') }}">Reset</a>
  </div>

  <div class="breadcrumb">
    <a href="{{ route('hands-on.index') }}">Browse</a>@if($type) / <a href="{{ route('hands-on.index') }}">Hands-On Tech</a>@endif
  </div>
  ...
  <div class="hero">
    ...
    <div class="topics">
      ...
      <div class="topic-row">
        <a class="topic {{ $type === 'challenge' ? 'active' : '' }}" href="{{ route('hands-on.index', ['type' => 'challenge']) }}">Hands-On Practice with Code Challenges</a>
        <a class="topic {{ $type === 'practice' ? 'active' : '' }}" href="{{ route('hands-on.index', ['type' => 'practice']) }}">Hands-On Practice with Cybersecurity Labs</a>
        <a class="topic {{ $type === 'project' ? 'active' : '' }}" href="{{ route('hands-on.index', ['type' => 'project']) }}">Hands-On Practice with GitHub Codespaces</a>
      </div>
    </div>
  </div>
  ...
  <div class="results-card">
    <div class="results-head">
      <div class="results-count">{{ $labs->total() }} Results for “{{ $heroTitle }}”</div>
      <div class="sort">
    ...
    @if($labs->count() > 0)
      <div class="list">
        @foreach($labs as $lab)
          <div class="row">
            <div class="thumb {{ $lab->type }}">{{ strtoupper(substr($lab->title,0,1)) }}</div>
            <div class="meta">
              <div class="kicker">{{ $lab->type_label }}</div>
              <div class="name">{{ $lab->title }}</div>
              <div class="sub">
                <b>{{ $lab->durasi_format }}</b> · {{ $lab->level }}
                @if($lab->teknologi) · {{ $lab->teknologi }} @endif
              </div>
              @if($lab->description)
                <div class="blurb">{{ $lab->description }}</div>
              @endif
            </div>
            <div class="actions">
      ...
      <div style="padding:16px 18px;border-top:1px solid #eef2f7">{{ $labs->links() }}</div>
    @else
