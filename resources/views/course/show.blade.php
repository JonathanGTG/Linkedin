...
<title>{{ $course->title }} - LinkedIn Learning</title>
...
<nav class="navbar">
  <div class="nav-left">
    <a href="{{ route('home') }}" class="hamburger" aria-label="Back to home">&#9776;</a>
    <a href="{{ route('home') }}" class="li-logo">in</a>
    <a href="{{ route('home') }}" class="nav-brand">Learning</a>
  </div>

  <div class="nav-right">
    <a class="nav-icon" href="{{ route('browse') }}" style="text-decoration:none;">
    ...
    <div class="nav-icon profile-wrapper" onclick="toggleProfile(event)">
      ...
      <div class="profile-dropdown" id="profileDropdown">
        ...
        <a href="{{ route('journey.index') }}" class="pd-item">My Career Plan</a>
        <a href="{{ route('library') }}" class="pd-item active">My Content</a>

        <div class="pd-divider"></div>

        <a href="{{ route('home') }}" class="pd-item">Home</a>
        <a href="{{ route('browse') }}" class="pd-item">Browse courses</a>
        ...
        <form method="POST" action="{{ route('logout') }}" onclick="event.stopPropagation()" style="margin:0">
          @csrf
...
<div class="hero-banner">
  @if($courseImage)
    <img src="{{ $courseImage }}" alt="" class="hero-bg" onerror="this.style.display='none'" />
  @else
  ...
  <div class="hero-content">
    <div class="hero-left">
      ...
      <div class="hero-meta">
        ...
        <span class="dot">&bull;</span>
        <span data-i18n="hero_updated">Updated: {{ $updatedDate }}</span>
      </div>
      ...
      <div class="hero-buttons">
        @if($enrollment)
          <span class="enrolled-status">
            ...
            {{ $enrollment->status === 'completed' ? 'Completed' : 'In your learning' }}
          </span>
          <a href="#chapters" class="btn-primary" data-i18n="btn_continue">Continue learning</a>
        @else
          <form method="POST" action="{{ route('library.enroll', $course) }}">
            @csrf
            <button class="btn-primary" data-i18n="btn_free">Start learning</button>
          </form>
        @endif
        <form method="POST" action="{{ route('library.save', $course) }}">
          @csrf
      ...
      @if($enrollment && $enrollment->status === 'completed')
        <form method="POST" action="{{ route('course.rate', $course) }}" class="rate-form">
          @csrf
          <div class="rate-row">
            <span class="rate-label">Your rating</span>
            <select name="rating" class="rate-select" required>
              @for($i = 5; $i >= 1; $i--)
                <option value="{{ $i }}" {{ $userRating && (int) $userRating->rating === $i ? 'selected' : '' }}>{{ $i }}</option>
              @endfor
            </select>
            <button class="btn-outline" type="submit">{{ $userRating ? 'Update rating' : 'Submit rating' }}</button>
          </div>
          <textarea name="review" class="rate-text" rows="3" placeholder="Optional review">{{ $userRating->review ?? '' }}</textarea>
        </form>
      @endif
    </div>
    ...
    <div class="hero-right">
      <div class="preview-card">
        <a class="preview-thumb-placeholder" href="#chapters" aria-label="Preview course">
          @if($courseImage)
            <img src="{{ $courseImage }}" alt="Preview" onerror="this.style.display='none'" />
          @endif
          <div class="play-btn">
...
<div class="main-layout">
  <div class="content-area">
    <div class="info-row">
      ...
      <span>{{ $projectFileCount > 0 ? $projectFileCount : 1 }} <span data-i18n="project_file">project file</span></span>
    </div>
    ...
    @if($enrollment && $totalVideos > 0)
      <div class="progress-box">
        <div class="progress-copy">
          <strong>{{ $progressPct }}% complete</strong><br>
    ...
    <p class="description">
      {{ $course->description ?: 'Build practical skills through guided lessons, hands-on examples, and a structured learning path designed for real work.' }}
    </p>

    <div class="section-title" data-i18n="skills_title">Skills covered in this course</div>

    <div class="tags">
      @forelse($course->skills as $skill)
        <span class="tag">{{ $skill->name }}</span>
      @empty
        <span class="tag">{{ $course->category->name ?? 'Professional Development' }}</span>
    ...
    <div class="instructor-list">
      @forelse($course->instructors as $instructor)
        <div class="instructor-card">
          <div class="instructor-avatar">{{ strtoupper(substr($instructor->name, 0, 1)) }}</div>
          <div>
            <div class="instructor-name">{{ $instructor->name }}</div>
            @if($instructor->info)
              <div class="instructor-info">{{ $instructor->info }}</div>
            @endif
            @if($instructor->link)
              <a class="instructor-link" href="{{ $instructor->link }}" target="_blank" rel="noopener">View profile</a>
            @endif
          </div>
        </div>
      @empty
        <div class="instructor-card">
          <div class="instructor-avatar">
            @if($course->instructor_avatar)
              <img src="{{ $course->instructor_avatar }}" alt="" onerror="this.style.display='none'" />
            @else
  ...
  <div class="right-panel" id="chapters">
    ...
    <div class="chapters">
      @forelse($course->chapters as $chapter)
        ...
        @endphp
        <div class="chapter {{ $loop->first ? 'active' : '' }}">
          <div class="chapter-header" onclick="toggleChapter(this)">
            <span>{{ $chapter->urutan }}. {{ $chapter->judul }}</span>
            <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          ...
          <div class="chapter-content">
            @foreach($chapterVideos as $video)
              @php $isDone = isset($videoProgress[$video->id]) && $videoProgress[$video->id]->is_completed; @endphp
              <div class="lesson {{ $isDone ? 'done' : '' }}" id="vrow-{{ $video->id }}">
                <div class="lesson-check">{!! $isDone ? '&#10003;' : e($video->urutan) !!}</div>
                <div class="lesson-info">
                  <div class="lesson-title">{{ $video->title }}</div>
                  <div class="lesson-duration">{{ $video->durasi_format }}</div>
                </div>
                @if($video->is_preview)
                ...
                @if($enrollment)
                  <button
                    class="lesson-complete {{ $isDone ? 'done' : '' }}"
                    onclick="markDone(event, '{{ $video->id }}', this)"
                    data-course="{{ $course->id }}"
                    data-durasi="{{ $video->durasi_detik }}">
            ...
            <div class="review-user">
              <div class="review-avatar">
                @if($commentUser && $commentUser->avatar)
                  <img src="{{ Str::startsWith($commentUser->avatar, ['http://', 'https://', '/']) ? $commentUser->avatar : asset($commentUser->avatar) }}" alt="" onerror="this.style.display='none'" />
                @else
                  {{ $commentInitial }}
                @endif
              </div>
              <div style="min-width:0">
                <div class="review-name">{{ $commentName }}</div>
                <div class="review-stars">
                  @for($i = 1; $i <= 5; $i++)
                    {!! $i <= (int) $comment->rating ? '&#9733;' : '&#9734;' !!}
                  @endfor
                </div>
              </div>
            </div>
            <div class="review-date">{{ $comment->created_at?->format('M j, Y') }}</div>
          </div>
          <div class="review-body">{{ $comment->review }}</div>
        </div>
      @endforeach
    </div>
  @endif
