...
@extends('layouts.app')
@section('title', $role['title'].' Role Guide - LinkedIn Learning')
...
<div class="role-page">
  <section class="role-hero" aria-label="Role guide overview">
    <div>
      <div class="role-eyebrow">{{ $role['eyebrow'] }}</div>
      <h1 class="role-title">{{ $role['title'] }}</h1>
      <p class="role-summary">{{ $role['summary'] }}</p>
      <div class="role-actions">
        <a class="primary-action" href="{{ route('journey.index') }}">Create my learning plan</a>
        <a class="secondary-action" href="{{ route('browse', ['type' => 'B', 'q' => $role['browse_query']]) }}">Browse related courses</a>
      </div>
    </div>
    <div class="hero-media" aria-label="{{ $role['title'] }} preview">
      <img src="{{ $role['hero_image'] }}" alt="{{ $role['title'] }} preview">
  ...
  <section class="role-section" aria-label="Role guide resources">
    <div class="section-head">
      <div>
        <h2 class="section-title">Top {{ $role['title'] }} skills</h2>
        <p class="section-note">Provided by LinkedIn.</p>
      </div>
      <a class="text-link" href="{{ route('browse', ['type' => 'B', 'q' => $role['browse_query']]) }}">Follow skills</a>
    </div>
    <div class="skill-count">{{ count($skills) }} skills to develop</div>
    <div class="skill-cloud">
      @foreach($skills as $skill)
        <a class="skill-pill" href="{{ route('roles.show', ['role' => $role['slug'], 'skill' => $skill]) }}">{{ $skill }}</a>
      @endforeach
    </div>
  ...
  <section class="role-section">
    ...
    <div class="card-rail">
      @foreach($shortVideos as $item)
        @include('roles.partials.learning-card', ['item' => $item])
      @endforeach
    </div>
  </section>
  ...
  <section class="role-section">
    ...
    <div class="skill-tabs" aria-label="Skill filters">
      @foreach($skills as $skill)
        <a class="skill-pill {{ $skill === $activeSkill ? 'active' : '' }}" href="{{ route('roles.show', ['role' => $role['slug'], 'skill' => $skill]) }}">{{ $skill }}</a>
      @endforeach
    </div>
    <div class="card-rail">
      @foreach($skillCourses as $item)
        @include('roles.partials.learning-card', ['item' => $item])
      @endforeach
    </div>

    <div class="cta-band">
      <p>Interested in becoming a {{ $role['title'] }}? Start by creating a learning plan and build the top skills with guided modules from your own course catalog.</p>
      <a class="primary-action" href="{{ route('journey.index') }}">Create my learning plan</a>
    </div>
  </section>
</div>
